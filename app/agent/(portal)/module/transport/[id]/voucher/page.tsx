...
export default async function AgentVoucherPage({ params, searchParams }: { params: { id: string }; searchParams: { brand?: string; fares?: string } }) {
  ...
  const provider = brand === "agent" && br
    ? { name: br.agency_name, tagline: null as string | null, contact: br.contact_person, mobile: br.mobile ?? agent.mobile, email: br.email ?? agent.email, address: br.address, logo: br.logo, logoHasName: !!br.logo_lockup, note: br.voucher_note }

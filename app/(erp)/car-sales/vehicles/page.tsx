...
export default async function VehiclesPage() {
  ...
  const { data } = await supabase
    .from("car_vehicles")
    .select("id, vehicle_no, vin, plate_no, make, model, variant, model_year, color, purchase_date, purchase_cost, purchase_vat, total_cost, status, ownership, current_location, supplier:supplier_id(name), customer:current_customer_id(name)")
    .order("created_at", { ascending: false })
    .limit(2000);

  const rows: VehicleRow[] = (data ?? []).map((v: any) => ({
    id: v.id, vehicle_no: v.vehicle_no, vin: v.vin, plate_no: v.plate_no,

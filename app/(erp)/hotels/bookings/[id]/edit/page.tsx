...
export default async function EditHotelBookingPage({ params }: { params: { id: string } }) {
  ...
  const [{ data: b }, { data: hotels }, { data: agents }, { data: staff }] = await Promise.all([
  ...
  return (
    <div className="max-w-4xl">
      ...
      <HotelBookingForm existing={b} hotels={(hotels ?? []) as any} agents={(agents ?? []) as any} salespeople={salespeople} />
    </div>
  );
}

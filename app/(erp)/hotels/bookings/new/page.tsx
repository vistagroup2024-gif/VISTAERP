...
export default async function NewHotelBookingPage() {
  ...
  const [{ data: hotels }, { data: agents }, { data: staff }] = await Promise.all([
  ...
  return (
    <div className="max-w-4xl">
      ...
      <HotelBookingForm existing={null} hotels={(hotels ?? []) as any} agents={(agents ?? []) as any} salespeople={salespeople} />
    </div>
  );
}

...
export default async function VoucherPage({ params, searchParams }: { params: { id: string }; searchParams: { brand?: string; fares?: string } }) {
  ...
  if (brand === "agent" && b.agent_id) {
    ...
    if (party || login) {
      agent = {
        ...
        voucher_show_name: login?.voucher_show_name ?? true,
      };
    }
  }
  ...
  const provider = brand === "agent" && agent
    ? { name: agent.agency_name, tagline: null as string | null, contact: agent.contact_person, mobile: agent.mobile, email: agent.email, address: agent.address, logo: agent.logo, note: agent.voucher_note, hideName: agent.voucher_show_name === false }

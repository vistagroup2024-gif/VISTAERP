...
export default async function PackageUpdatesPage({ searchParams }: { searchParams: { company?: string } }) {
  ...
  const R = rows ?? [];
  // Fetch consumptions for these groups to compute true uncovered nights
  ...
  const items = R.map((g: any) => {
    ...
    if (g.package_status !== "updated" && daysLeft !== null) {
    ...
    <div>
      ...
      <div className="mt-4 card overflow-x-auto p-0">
        <table className="w-full min-w-[1120px]">
          ...
          <tbody>
            {items.map((i) => (
              <tr key={i.g.id} className={`border-t border-slate-100 ${i.priority === "Overdue" && i.g.package_status !== "updated" ? "bg-red-50" : ""}`}>
                ...
                <td className={`td font-semibold ${i.remaining > 0 ? "text-orange-600" : "text-green-600"}`}>{i.remaining}</td>
                <td className="td text-sm">{i.deadline ? dateStr(i.deadline) : "—"}{i.daysLeft !== null && i.g.package_status !== "updated" ? ` (${i.daysLeft}d)` : ""}</td>
                <td className="td">{i.g.package_status !== "updated" ? <span className={`badge ${i.pcls}`}>{i.priority}</span> : <span className="text-slate-300">—</span>}</td>
            ...
            {items.length === 0 && <tr><td className="td text-slate-400" colSpan={11}>No packages need updates. 🎉</td></tr>}
          </tbody>
        </table>
      </div>
    </div>

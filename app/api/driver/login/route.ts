...
export async function POST(req: Request) {
  ...
  const { data, error } = await supabase.rpc("login_driver", { p_username: username, p_password: password });
  if (error || !data) return NextResponse.json({ error: error?.message ?? "Invalid username or password" }, { status: 401 });
  const res = NextResponse.json({ ok: true });
  res.cookies.set("driver_session", data as string, { httpOnly: true, sameSite: "lax", secure: true, path: "/", maxAge: 60 * 60 * 24 * 7 });
  return res;
}

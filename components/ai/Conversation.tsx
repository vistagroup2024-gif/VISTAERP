...
// ============================================================
// The conversation itself — the whole machine, in one place.
//
// Shared by the full /ai screen and the dock that opens over any ERP page,
// so there is one implementation of asking, streaming, speaking and
// interrupting rather than two that drift apart.
// ============================================================
...
export default function Conversation({
  ...
  configured: boolean;
  /** The ERP screen the user is on, or null when they are on /ai itself. */
  page?: PageContext | null;
  /** Reports what she is doing, so a parent can drive the avatar. */
  onState?: (state: ConversationState, level: number) => void;
  compact?: boolean;
}) {
  ...
  // The page can change under a dock that stays open while the user navigates.
  ...
  useEffect(() => { onState?.(state, voice.level); }, [state, voice.level, onState]);
    ...
    <>
      {!configured && (
        <div className="border-b border-warning/20 bg-warning-soft px-5 py-3 text-sm text-warning-fg">
          Vista AI has no API key configured on the server yet, so she cannot answer. An administrator
          needs to set <span className="font-mono text-xs">ANTHROPIC_API_KEY</span>.
        </div>
      )}
      {voice.error && (
      ...
      {/* What she is looking at with you, when she is open over a screen. */}
      {page?.screen && turns.length === 0 && (
        <div className="border-b border-slate-100 px-4 py-2 text-xs text-slate-500">
          Looking at <span className="font-medium text-slate-700">{page.screen}</span>
          {page.entityId ? " — ask about “this” and I'll read the record open here." : ""}
        </div>
      )}
      ...
      >
        ...
        <textarea
          ...
          rows={1}
          placeholder={settings.hands_free && voice.listening ? "Listening — just speak…" : "Ask Vista AI…"}
          value={input}
          disabled={!configured}
        ...
        {busy || voice.speaking ? (
          <button type="button" onClick={() => { stop(); voice.hush(); }} className="btn-outline shrink-0">
            Stop
          </button>
        ) : (
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-slate-100 px-4 py-2 text-[11px] text-slate-400">
        ...
        {!compact && <span>Everything she quotes is read live from the ERP under your own permissions.</span>}
      </div>
    </>
  );
}

...
interface Item { href: string; label: string; icon?: IconName; perm?: string[]; exact?: boolean }
// `perm` (any-of) gates the whole module. Omit to always show.
interface Group { label: string; icon: IconName; items: Item[]; perm?: string[] }

export interface StaffNavAccess { unrestricted: boolean; permissions: Record<string, boolean> }

// Standalone top link (no parent).
const DASHBOARD: Item = { href: "/dashboard", label: "Dashboard", icon: "dashboard" };

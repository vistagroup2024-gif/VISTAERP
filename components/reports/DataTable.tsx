...
export default function DataTable({
  ...
  const filteredGroups = useMemo(() => {
    if (!groups) return [];
    return groups.map((g) => ({ ...g, rows: g.rows.filter(matches) })).filter((g) => g.rows.length > 0 || !q.trim());
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [groups, q]);
  ...
  const rowCount = isFlat ? flatRows.length : filteredGroups.reduce((s, g) => s + g.rows.length, 0);
  ...
  return (
    <tbody>
      {groups.map((g) => {
        ...
        return (
          <Fragment key={g.key}>
            <tr className="cursor-pointer border-t-2 border-slate-200 bg-slate-50 font-semibold" onClick={() => onToggle(g.key)}>
            ...
            {open && g.subtotal && (
              <tr key={`${g.key}-sub`} className="border-t border-slate-200 bg-slate-50/60 font-medium">
                {cols.map((c, i) => (
                  <td key={c.key} className={`px-3 py-1.5 ${isNumeric(c) ? "text-right tabular-nums" : ""}`}>

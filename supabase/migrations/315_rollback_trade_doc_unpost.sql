-- ROLLBACK for 315_trade_doc_unpost.sql.
--
-- Puts the lock back: a posted trade voucher refuses to be deleted, and saving
-- one falls through to trade_doc_post, which raises 'Already posted'. Also
-- restores trade_doc_save to totalling from the line amounts alone, i.e. the
-- state before BOTH 314 and 315.
--
-- Safe to run at any time. It does NOT re-post anything a user unposted while
-- 315 was in place — an unposted voucher stays unposted with status 'open', and
-- is re-posted by opening and saving it again.
--
-- trade_doc_unpost itself is left in place but unreachable: nothing calls it
-- once these two bodies are back, and it is granted to nobody. Drop it by hand
-- if you want it gone:
--     drop function if exists trade_doc_unpost(uuid, text);
--     drop function if exists staff_require_doc_strict(text, text);
--     drop function if exists staff_doc_right_strict(text, text);

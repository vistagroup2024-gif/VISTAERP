-- 221_trip_ledger_completed_only.sql
-- Transport Trip Ledger: show ONLY completed trips.
--
-- Previously every non-cancelled scheduled trip in the date range appeared
-- (pending, assigned or completed). The ledger is a settlement/invoicing view,
-- so it should list a trip only once the trip has actually been completed. Add
-- `ts.status = 'completed'` to the filter; everything else is unchanged.

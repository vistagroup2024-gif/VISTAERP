-- The public voucher shows the ORIGINALLY BOOKED vehicle (requested), never a Vista
-- upgrade — the invoice/voucher follows the booked car; the upgrade only shows in Ops.
-- (public_transport_voucher recreated with v joined on coalesce(requested_vehicle_id, vehicle_id).)

-- 222_trip_ledger_latest_first.sql
-- Transport Trip Ledger: order latest trip date first (newest on top).
-- Only the ORDER BY changes; the completed-only filter and all columns stay.
...
begin
  ...
  order by ts.trip_date desc, ts.trip_time desc;
end $function$;

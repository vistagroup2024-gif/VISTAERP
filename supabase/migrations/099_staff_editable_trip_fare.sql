-- ============================================================
-- VISTA ERP - 099 Staff-editable per-trip fare
-- On single/multiple bookings, transport_save_booking and transport_sync_trips
-- now honour a staff-supplied per-trip sell_rate override from the booking form
-- (instead of always recomputing from the route/agent rate master). Package
-- bookings are unchanged: priced at package level, then distributed across trips.
...
begin
  ...
  if v_type = 'package' and v_pkg is not null then
    v_sell := coalesce(transport_package_price(v_company, v_agent, v_pkg, v_pkg_veh), 0);
    ...
    for x in select * from jsonb_array_elements(coalesce(p_trips,'[]'::jsonb)) loop
      -- staff may override the per-trip fare; else fall back to the agent rate.
...
begin
  ...
  for t in select * from jsonb_array_elements(coalesce(p_trips, '[]'::jsonb)) loop
    ...
    -- Non-package bookings honour a staff-supplied per-trip fare override;
    -- package trips are always priced from the route master then distributed.
    ...
    if v_tveh is not null and v_pax is not null then
      ...
      if v_cap is not null and v_pax > v_cap then
        raise exception 'Vehicle capacity (%) is less than total passengers (%). Choose a larger vehicle.', v_cap, v_pax;
      end if;
    end if;

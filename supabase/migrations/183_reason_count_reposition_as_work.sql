-- Repositioning (deadhead) driving between chained trips is real driving but was
-- NOT counted toward the 12h working limit. That let the engine chain e.g. a Madinah
-- Ziarat then a Makkah->Madinah on one driver: 2.5h + 5h trips = 7.5h "work" passed,
-- even though the driver must also drive 5h Madinah->Makkah between them (12.5h real
-- driving). Now the reposition minutes are added to the working total, so such
-- impossible chains are rejected.
  ...
  for i in lo..hi loop
    if i > lo then
      ...
      if arr_to[i-1] is not null and arr_from[i] is not null
        ...
        -- Repositioning is real driving: count it toward the 12h working limit.

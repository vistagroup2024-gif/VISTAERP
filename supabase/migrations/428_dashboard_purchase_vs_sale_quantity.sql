-- Purchase vs Sale is an inventory-movement card, not a transaction-count
-- card: migration 427's purchase_txns_month/sale_txns_month were still
-- counting DOCUMENTS, which is not what "purchase vs sale" means for a
-- business that trades physical units (goods and cars). Replaced with real
-- QUANTITY off the stock ledger: purchased_qty_month / sold_qty_month reuse
-- the exact in/out logic stock_statement() already verifies per item
-- (qty>0 = received, qty<0 = issued off stock_movements), summed
-- company-wide instead of broken out per item; remaining_qty is the same
-- qty stock_balances already carries on the Stock card. No new business
-- rule — the same ledger every Inventory report already reads, aggregated
-- differently.
...
with
  ...
  -- Company-wide unit movement — the same in/out logic stock_statement()
  -- already uses per item (qty>0 = received, qty<0 = issued off
  -- stock_movements), just summed across every stock item (goods AND cars —
  -- a car posts through this same ledger, one unit = one movement row) for
  -- the current month, instead of broken out per item for an arbitrary
  -- range. not p.is_group excludes a product-tree header row the same way
  -- stock_statement() does; every row in stock_movements is already a real
  -- stock item by construction (is_stock_item()'s own definition), so no
  -- further filter is needed.
  stock_moves as (
    select
...
select jsonb_build_object(
  ...
  -- Purchase vs Sale is an INVENTORY MOVEMENT card, not a second P&L or a
  -- transaction-count card: purchased/sold/remaining QUANTITY, off the same
  -- stock ledger stock_statement() already reports per item, summed
  -- company-wide (goods and cars both, since a car is one physical unit on
  -- the same ledger). Sales and P&L already carry the money side.
...
declare
  ...
  v_result jsonb;
  v_pvs jsonb;
  v_purchased numeric; v_sold numeric; v_remaining numeric;
  v_stmt jsonb;
  v_stmt_in numeric; v_stmt_out numeric;
  v_direct_stock_qty numeric;
...
begin
  ...
  v_purchased := (v_pvs->>'purchased_qty_month')::numeric;
  v_sold := (v_pvs->>'sold_qty_month')::numeric;
  v_remaining := (v_pvs->>'remaining_qty')::numeric;

  -- Independent cross-check: the already-verified stock_statement() RPC,
  -- summed across every item it returns for the same month-to-date window,
  -- must agree with the new company-wide totals — two different code paths
  -- over the same ledger, not the same query typed twice.
  select public.stock_statement(v_month_start, v_today) into v_stmt;
  select coalesce(sum((x->>'in_qty')::numeric), 0), coalesce(sum((x->>'out_qty')::numeric), 0)
    into v_stmt_in, v_stmt_out
  from jsonb_array_elements(v_stmt) x;

  if v_purchased <> v_stmt_in then
    raise exception 'purchased_qty_month self-check: % does not match stock_statement() sum %', v_purchased, v_stmt_in;
  end if;
  if v_sold <> v_stmt_out then
    raise exception 'sold_qty_month self-check: % does not match stock_statement() sum %', v_sold, v_stmt_out;
  end if;

  select coalesce(sum(b.qty), 0) into v_direct_stock_qty
  from stock_balances b where b.company_id = v_company;
  if v_remaining <> v_direct_stock_qty then
    raise exception 'remaining_qty self-check: % does not match direct stock_balances sum %', v_remaining, v_direct_stock_qty;
  end if;

  raise notice 'dashboard_metrics purchase_vs_sale self-check passed: purchased_qty_month=%, sold_qty_month=%, remaining_qty=%', v_purchased, v_sold, v_remaining;
end;
$chk$;

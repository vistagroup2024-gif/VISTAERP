-- 277_sales_invoice_posting.sql
-- The Sales Invoice is the selling counterpart of the Purchase Voucher: it is
-- what turns a Sale Order into revenue and a receivable, and it is the document
-- that takes the goods off the shelf.
--
--   Dr Customer            total
--     Cr Sales             total
--   Dr Cost of Goods Sold  cost of the stock lines, at moving average
--     Cr Inventory         same
--
-- The Delivery Note that follows records what physically left and moves no
-- stock — the invoice already did, so the two can never double-count. That is
-- why the Delivery Note hides rate and amount.

-- The dashboard's Cash & Bank card, and now the new Cash & Bank report, both
-- read accounts by subtype IN ('Cash','Bank') — but that subtype was left
-- null on most of the real bank/cash accounts: every named driver's
-- "CASH IN HAND (...)" account and most of the named bank accounts under
-- BANK (RIYAD/RAJHI/ALINMA...) carry no subtype at all. Today's balance
-- happens to be unaffected only because none of the untagged accounts have
-- ever been posted to — the first receipt into one of them would silently
-- vanish from both the card and the report, the same undercounting shape
-- CLAUDE.md's September 2026 sweep already found and fixed for AR/AP.
--
-- Fixing every account's subtype by hand is a per-row guess this migration
-- should not make. The chart already has a reliable hierarchy — grouping is
-- exactly what CASH IN HAND (1-02) and BANK (1-03) already are — so this
-- gives them one parent, "CASH & BANK", and both the dashboard card and the
-- new report read the ACCOUNT SUBTREE from here on, falling back to subtype
-- only for an account tagged Cash/Bank somewhere else in the chart. Nothing
-- is renumbered: 1-02 and 1-03 keep their own codes (car_post_entry and
-- friends resolve accounts by code), only their parent_id changes, and
-- acct_rebuild_paths() is what the account tree's own multi-move (Masters
-- ACCT_MOVE_MANY) already calls to keep every descendant's materialized path
-- correct after a reparent — the same routine, not a hand-rolled UPDATE.

do $setup$
declare
  v_company uuid := '96f6b539-b491-4df7-91a2-80c7c8e7491d';
  v_assets  uuid := '56117a79-e7cf-4700-b01c-b103cd1c1cb2';
  v_cash    uuid := 'd5bd3604-d01d-4c06-b5bb-6054781b17c6';   -- 1-02 CASH IN HAND
  v_bank    uuid := 'e9567e68-025d-46a6-958b-8efcdf8c355d';   -- 1-03 BANK
  v_group   uuid;
begin
  if exists (select 1 from accounts where company_id = v_company and code = '1-CB') then
    select id into v_group from accounts where company_id = v_company and code = '1-CB';
  else
    insert into accounts (company_id, code, name, type, is_group, is_postable, parent_id, sort_order)
    values (v_company, '1-CB', 'CASH & BANK', 'asset', true, false, v_assets, 15)
    returning id into v_group;
  end if;

  update accounts set parent_id = v_group where id in (v_cash, v_bank) and parent_id is distinct from v_group;

  perform acct_rebuild_paths(v_company);
end;
$setup$;

-- dashboard_metrics(): cash_bank and cash_flow now read the account SUBTREE
-- under CASH & BANK (matched by its stable code, not its name, so a rename
-- on the tree screen does not silently stop counting), OR'd with the old
-- subtype tag so an account tagged Cash/Bank anywhere else in the chart
-- still counts. Every other block is untouched, copied verbatim.
...
with
  ...
  -- Matched by CODE, not name, so a rename on the tree screen does not
  -- silently stop counting. cash_grp/bank_grp are the two ORIGINAL groups
  -- (1-02, 1-03) so "cash" vs "bank" still splits the same way it always
  -- did, only now by the account's real position in the tree rather than a
  -- subtype text that was never set on most of them.
...
begin
  select id, path into v_group, v_group_path from accounts where company_id = v_company and code = '1-CB';
  if v_group is null then raise exception 'cash_and_bank self-check: CASH & BANK group was not created'; end if;

  select parent_id into v_cash_parent from accounts where id = 'd5bd3604-d01d-4c06-b5bb-6054781b17c6';
  select parent_id into v_bank_parent from accounts where id = 'e9567e68-025d-46a6-958b-8efcdf8c355d';
  if v_cash_parent is distinct from v_group or v_bank_parent is distinct from v_group then
    raise exception 'cash_and_bank self-check: CASH IN HAND / BANK were not reparented under CASH & BANK';
  end if;

  -- A previously-untagged descendant (a driver's cash-in-hand account, no
  -- subtype) must now carry a path under the new group — proves
  -- acct_rebuild_paths() actually cascaded, not just the two direct children.
  select path into v_fiaz_path from accounts where id = 'd6d3b2a4-71b3-4f9e-bee7-1c16a73e6fcf';
  if v_fiaz_path is null or v_fiaz_path not like v_group_path || '/%' then
    raise exception 'cash_and_bank self-check: descendant path was not rebuilt (got %, want under %)', v_fiaz_path, v_group_path;
  end if;

  perform set_config('request.jwt.claims', json_build_object('sub', v_admin::text)::text, true);

  select coalesce(sum(l.debit - l.credit), 0) into v_old_balance
    from journal_lines l join journal_entries e on e.id = l.entry_id join accounts a on a.id = l.account_id
   where e.status = 'posted' and e.company_id = v_company and a.subtype in ('Cash', 'Bank');

  select (public.dashboard_metrics() -> 'cash_bank' ->> 'balance')::numeric into v_new_balance;

  -- The new, more-inclusive definition must never read LESS than the old
  -- subtype-only one — only equal or more, since it is a strict superset of
  -- accounts. On today's data (nothing was ever posted to an untagged
  -- account) the two are expected to be exactly equal.
  if v_new_balance < v_old_balance - 0.01 then
    raise exception 'cash_and_bank self-check: new balance % is less than the old subtype-only balance %', v_new_balance, v_old_balance;
  end if;

  raise notice 'cash_and_bank self-check passed: group=%, old_subtype_balance=%, new_balance=%', v_group, v_old_balance, v_new_balance;
end;
$chk$;
